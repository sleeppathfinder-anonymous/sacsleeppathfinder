...
# ==================== 연구자 설정 ====================
# 실험 조건 기본값: EXP (환경변수 ASSIGNED_ARM 로 EXP/CTRL 전환 가능)
ASSIGNED_ARM = os.getenv("ASSIGNED_ARM", "EXP").upper().strip()
...
def assign_conditions(state):
    state["arm"] = ASSIGNED_ARM if ASSIGNED_ARM in {"EXP", "CTRL"} else "EXP"
    if state["arm"] == "EXP":
...
def user_input_handler(user_input, state):
    ...
    if current_stage == STAGE_PSYCHOEDUCATION_START:
        ...
        if new_state["arm"] == "EXP":
            ...
            retrieved_rag_info = retrieve_rag_info(problem_summary_text, new_state.get('predicted_label', "Sleep Hygiene"), new_state)
            prompt_content = PROMPTS_EXP["rq2_rag_enhanced_prompt"].replace("{{predicted_label}}", new_state.get('predicted_label', "Sleep Hygiene")).replace("{{problem_summary}}", problem_summary).replace("{{retrieved_info}}", retrieved_rag_info)
        ...
        # EXP 조건에서는 최종 확인 메시지를 이미 _end_session에서 생성·출력
        final_message = "좋아요. 오늘 나눈 내용을 바탕으로 가능한 범위에서 천천히 시도해 보시면 좋겠습니다."
